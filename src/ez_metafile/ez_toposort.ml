      ...
      let sort list =
        ...
        while !add_edges != [] do
          ...
          | (node, t, t_node) :: tail ->
              add_edges := tail;
              add t t_node;
              if not (IntMap.mem node.node_id t_node.node_incoming_edges) then
                begin
                  t_node.node_incoming_edges <-
                    IntMap.add node.node_id node t_node.node_incoming_edges;
                  (*                DEBUG.printf 11 "Adding edge %d:%s
                                    -> %d:%s" node.node_id
                                    node.node_name t_node.node_id
                                    t_node.node_name; *)
        ...
        IntMap.iter (fun _ (_,node) ->
            (*            DEBUG.printf 11 "Total %d (%d)" node.node_id
