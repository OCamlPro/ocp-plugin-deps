...
let load_config ?(ocamlfind=["ocamlfind"]) () =
  try
    match
      EZCALL.call_stdout_lines @@ ocamlfind @ [ "printconf" ]
    with exception e -> raise e
    | lines ->
        ...
        and iter_path lines =
          match lines with
          | path :: lines when EzString.starts_with path ~prefix:"    " ->
              search_path := String.sub path 4
                  (String.length path - 4) :: !search_path;
              iter_path lines
          | _ -> iter lines
