 ...
 * Unix: a limited version of the Unix library, where non-portable
   calls have been replaced by the `unimplemented` type.

 * EzUnix: a version of Unix working only a Unix systems, not Win32,
...
[@@@yalo.warning "YALO-10-8-33-9-28,ZANUDA-1"]
...
*)
  ...
  let open_proc cmd optenv proc stdin stdout stderr =
    ...
    let pid =
      create_process_chdir shell
        ~cmd:(Printf.sprintf "%s /c %s" shell cmd) ?env:optenv
